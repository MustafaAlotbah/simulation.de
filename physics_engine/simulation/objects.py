...
class Particle:
    ...
    def get_forces_quivers(self):
        # maybe the set of force
        if not self.display_forces:
            return [], [], []
        forces_offset = [self.position.tolist() for _ in self.forces]
...
class Spring:
    def __init__(self, particle1: Particle, particle2: Particle, stiffness=6000.0, length=None, damping=1):
